...
import { getAnnouncementById, updateAnnouncement } from '@/api/announcement'
...
const announcementId = ref(Number(route.params.id))
...
const submitForm = async () => {
  ...
  try {
    ...
    await updateAnnouncement(announcementId.value, {
...
const loadAnnouncement = async () => {
  try {
    const data = await getAnnouncementById(announcementId.value)
    form.value.a_title = data.a_title
    form.value.a_content = data.a_content
    // 处理接收者数据
    if (data.receivers) {
      form.value.receiver_student_ids = data.receivers.map((receiver: any) => receiver.student_id)
    }
  } catch (error) {
