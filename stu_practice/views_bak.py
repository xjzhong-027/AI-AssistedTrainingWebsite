...
def practice_list(request):
    ...
    for practice in practices:
        ...
        # 获取当前学生在该练习中的记录
        student_practice_record = StudentExamRecord.objects.filter(user=student, practice=practice).first()
...
def practice_page(request, practice_id, order):
    ...
    student = get_object_or_404(Students, username=username)
    student_practice_record = get_object_or_404(StudentExamRecord, user=student, practice=practice)
...
def practice_result(request, practice_id):
    ...
    student_practice_record = get_object_or_404(StudentExamRecord, practice=practice)
    page_records = StudentPageRecord.objects.filter(student_practice_record_id=student_practice_record.id)
